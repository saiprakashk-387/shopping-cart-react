const API = [
  // {
  //   name: "Kiwi",
  //   price: 19.99,
  //   quantity: 1,
  //   inCart: false,
  //   src:
  //     "https://images.pexels.com/photos/51312/kiwi-fruit-vitamins-healthy-eating-51312.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500"
  // },
  // {
  //   name: "Watermelon",
  //   price: 14.99,
  //   quantity: 1,
  //   inCart: false,
  //   src:
  //     "https://images.pexels.com/photos/1313267/pexels-photo-1313267.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500"
  // },
  // {
  //   name: "Cherry",
  //   price: 8.99,
  //   quantity: 1,
  //   inCart: false,
  //   src:
  //     "https://images.pexels.com/photos/109274/pexels-photo-109274.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500"
  // },
  // {
  //   name: "Avacado",
  //   price: 6.99,
  //   quantity: 1,
  //   inCart: false,
  //   src:
  //     "https://images.pexels.com/photos/557659/pexels-photo-557659.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500"
  // },
  // {
  //   name: "Apple",
  //   price: 5.99,
  //   quantity: 1,
  //   inCart: false,
  //   src:
  //     "https://images.pexels.com/photos/102104/pexels-photo-102104.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500"
  // },
  // {
  //   name: "Strawberry",
  //   price: 11.99,
  //   quantity: 1,
  //   inCart: false,
  //   src:
  //     "https://images.pexels.com/photos/934066/pexels-photo-934066.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500"
  // },
  // {
  //   name: "Orange",
  //   price: 3.99,
  //   quantity: 1,
  //   inCart: false,
  //   src:
  //     "https://images.pexels.com/photos/42059/citrus-diet-food-fresh-42059.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500"
  // }
  {
    name: 'dress', 
    // cloth_type: 'nylon',
     price: 550,
     quantity: 1,
     src: 'http://res.cloudinary.com/dignfufky/image/upload/v1658308516/hwy3artq7jbcohxaxggf.jpg',
     },
     
    {quantity: 1, name: 'hs',
    //  cloth_type: 'polyster',
      price: 650, src: 'http://res.cloudinary.com/dignfufky/image/upload/v1658308086/ywwyr2ul84htfzfwla7e.jpg'},
    
    {quantity: 1, name: 'test', 
    // cloth_type: 'col2', 
    price: 200, src: 'http://res.cloudinary.com/dignfufky/image/upload/v1658308516/hwy3artq7jbcohxaxggf.jpg'}
];
export default API;
 const api =[
  
  {
  name: 'dress', 
  // cloth_type: 'nylon',
   price: 550,
   quantity: 1,
   src: 'http://res.cloudinary.com/dignfufky/image/upload/v1658308516/hwy3artq7jbcohxaxggf.jpg',
   },
   
  {quantity: 1, name: 'hs',
  //  cloth_type: 'polyster',
    price: 650, src: 'http://res.cloudinary.com/dignfufky/image/upload/v1658308086/ywwyr2ul84htfzfwla7e.jpg'},
  
  {quantity: 1, name: 'test', 
  // cloth_type: 'col2', 
  price: 200, src: 'http://res.cloudinary.com/dignfufky/image/upload/v1658308516/hwy3artq7jbcohxaxggf.jpg'}
  ]